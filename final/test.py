...
class gameState:
    ...
    def update(self):
        self.kid_pos = self.new_kid_pos
        if self.new_kid_pos != None and self.new_barrier_pos != None:
            for ba in self.new_barrier_pos:
                if self.new_kid_pos[1]+SIDE == ba[2]:
...
while True: 
    ...
    def Max_Value (game, d):
        ...
        for action in game.legalmove() :
            ...
            state = [gameState1.new_kid_pos, gameState1.new_barrier_pos, gameState1.kid_pos, gameState1.barrier_pos, hell.end, action]
